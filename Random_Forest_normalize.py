...
# lon, latの値を小数第3位で切り捨てる
dataset.lon = round(dataset.lon, 2)
dataset.lat = round(dataset.lat, 2)
dataset_valid.lon = round(dataset_valid.lon, 2)
dataset_valid.lat = round(dataset_valid.lat, 2)

# print("dataset_lat: \n", dataset.lat)

print("dataset: ", dataset)
print("index: ", dataset.columns)
print("dataset.dtype: ", dataset.dtypes)
# print("date_dataset: ", dataset[dataset.date == '20210101'])
print("length: ", dataset.shape)
print("date_type: ", type(dataset.date))
# print("lon_max, lon_min: ", dataset.lon.max(), dataset.lon.min())
print("lat_max, lat_min: ", dataset.lat.max(), dataset.lat.min()) 

  

# 先に大きな枠組みを作る(numpyで)  1445129 * 820
# そこに代入していく方式がいいかも

# c = dataset.date.value_counts(dropna=False, sort=False)
# print("date: \n", c)
...
# ate_valid_length = 0
# start = time()
# for i in dataset_valid.date:
#   print(i)
#   date_valid_length = date_valid_length + 1

# end = time()
# cost = np.round( (end - start)/60 , 2)
# print(f"Finished in : {cost} minutes" )

# print("dataset.date.length", date_length)
# print("dataset_valid.date.length", date_valid_length)
# print("dataset.shape: ", dataset.shape)
# print("dataset_valid.shape: ", dataset_valid.shape)

 
# 日にちごとに分割(それぞれで一回保存する)
start = time() # 実行時間の確認
new_dataset = np.zeros((1445129, 820)) # 空のデータセット, 全部終わった後にpdに変換する
n = 0 # datasetにvectorを代入していく際のカウンター
# train datasetを作り直す
for i in dataset.date:

  start_date = time() # 1日ごとの実行時間
  dataset_A = dataset[dataset.date == i]# ある特定の日だけのデータセットdataset_A
  # print(f"{i}: {dataset_A.shape}")
  # print("dataset[dataset['date'] == i]: \n", dataset_A)
  
  # print("dataset_date: ", i)

  j = dataset.iat[n, 0] # 特定の日の基点
  k = dataset.iat[n, 1]
  # print("j: ", j)
  # print("k: ", k)
  new_vector = np.array([]) #空のベクトル(ここで初期化) 92 * 9 - 8 = 820
  
  # start_lonLat = time() # 各地点の周囲の処理の実行時間
  # print("基点の緯度経度j, k: ", j, k)
  x_list = [round(j - 0.45, 2), j, round(j + 0.45, 2)]
  y_list = [round(k - 0.45, 2), k, round(k + 0.45, 2)]
  # print("x_list: ", x_list)
  # print("y_list: ", y_list)
  for x in x_list:
    for y in y_list:
      # print("x, y: ", x, y)
      # print("lat_list: \n", dataset_A[dataset_A.lon == x].lat)
      # xlon_list = dataset_A.lon.values.tolist() # xの取り得る値をリスト化
      # ylat_list = dataset_A[dataset_A.lon == x].lat.values.tolist() # yの取り得る値をリスト化
      # print("xlon_list: ", xlon_list)
      # print("ylat_list: ", ylat_list)
      
      dataset_D = dataset_A[dataset_A.lon == x]
      # print("dataset_D: \n", dataset_D)
      # print("dataset_D.lat: ", dataset_D.lat)
      # print("lat = 10.15: ", dataset_D[dataset_D.lat == 10.15])
      # print("y: ", round(y, 2))

      dataset_D = dataset_D[dataset_D.lat == round(y, 2)] # ある地点(k, j)の周辺, yの値を小数第3位で切り捨て
      # print("dataset_D: \n", dataset_D)
      
      if dataset_D.size == 0: # 周囲の地点が存在しない場合(dataset_Dが空なら)
        dataset_D = np.zeros((1, 92)) # 各要素を0にする
        dataset_D[0, 0] = x
        dataset_D[0, 1] = y
        dataset_D[0, 90] = i # 日にち、緯度、経度は代入
      
      # dataset_D = dataset_D.values
      
      dataset_D = np.nan_to_num(dataset_D)
      # dataset_D = dataset_D.fillna(0) # 欠損値(NaN)を0に置換
      
        
      if new_vector.size == 0: # ベクトルが空なら
        new_vector = dataset_D # ベクトルが空なら代入
      else: # 空じゃないなら
        # print("dataset_D: ", dataset_D)
        dataset_D = np.delete(dataset_D, 90, 1) # 日にちがダブるから削除
        dataset_D = dataset_D.reshape([1, 91]) # dataset_D を(0, 91)から(1, 91)にする
        # print("dataset_D.shape: ", dataset_D.shape)
        # print("new_vector.shape: ", new_vector.shape)
        new_vector = np.concatenate([new_vector, dataset_D], axis = 1)
        # print("new_vector.shape: ", new_vector.shape)
        # new_vector = pd.concat([new_vector, dataset_D], axis = 1) # 空じゃないなら結合
        
      # print("\n")

  # print("new_vector.shape: ", new_vector.shape)
  new_dataset[n, :] = new_vector # 新しく作ったベクトルをデータセットに代入する
  n = n + 1
      
  # end_lonLat = time()
  # cost_lonLat = np.round( (end_lonLat - start_lonLat)/60 , 2)
  # print(f"dataset.date = {i} (Lon_Lat) Finished in : {cost_lonLat} minutes" ) # 各地点の処理時間の長さ
  
  end_date = time()
  cost_date = np.round( (end_date - start_date)/60 , 2)
  # print(f"({i})Finished in : {cost_date} minutes" )
  # print("n: ", n)
...
pickle.dump(new_dataset, open('new_dataset_Spatial_Correlation.pkl', 'wb')) 
'''

dataset = pickle.load(open('new_dataset_Spatial_Correlation.pkl', 'rb')) # 新しい方のdataset
...
# 日にちごとに分割(それぞれで一回保存する)
start = time() # 実行時間の確認
new_dataset_valid = np.zeros((2822310, 820))
n = 0 # datasetに代入する際に使うカウンター
for i in dataset_valid.date:

  start_date = time() # 1日ごとの実行時間
  dataset_valid_A = dataset_valid[dataset_valid.date == i] # ある特定の日だけのデータセットdataset_A
  # print("dataset[dataset['date'] == i]: \n", dataset_A)
  
  print("dataset_valid_date: ", i)

  for j in dataset_valid_A.lon: # 緯度
    # print("lon: ", j)
    dataset_valid_B = dataset_valid_A[dataset_valid_A.lon == j] # 日と緯度を限定したデータセットdataset_B(for文を回すためだけ)
    for k in dataset_valid_B.lat: # 経度
      # print("lat: ", k)
      new_vector = np.array([]) #空のベクトル(ここで初期化) 92 * 9 - 8 = 820
      ...
      for x in x_list:
        for y in y_list:
          # print("x, y: ", x, y)
          # print("lat_list: \n", dataset_A[dataset_A.lon == x].lat)
          # xlon_list = dataset_valid_A.lon.values.tolist() # xの取り得る値をリスト化
          # ylat_list = dataset_valid_A[dataset_valid_A.lon == x].lat.values.tolist() # yの取り得る値をリスト化
          # print("xlon_list: ", xlon_list)
          # print("ylat_list: ", ylat_list)

          
          
          dataset_valid_D = dataset_valid_A[dataset_valid_A.lon == x]
          # print("dataset_D: \n", dataset_D)
          # print("dataset_D.lat: ", dataset_D.lat)
          # print("lat = 10.15: ", dataset_D[dataset_D.lat == 10.15])
          # print("y: ", round(y, 2))

          dataset_valid_D = dataset_valid_D[dataset_valid_D.lat == round(y, 2)] # ある地点(k, j)の周辺, yの値を小数第3位で切り捨て
          # print("dataset_D: \n", dataset_D)
          
          if dataset_valid_D.size == 0: # 周囲の地点が存在しない場合(dataset_D_validが空なら)
            dataset_valid_D = np.zeros((1, 92)) # 各要素を0にする
            dataset_valid_D[0, 0] = x
            dataset_valid_D[0, 1] = y
            dataset_valid_D[0, 90] = i # 日にち、緯度、経度は代入
          
          # dataset_D = dataset_D.values
          
          dataset_valid_D = np.nan_to_num(dataset_valid_D)
          # dataset_D = dataset_D.fillna(0) # 欠損値(NaN)を0に置換
          
            
          if new_vector.size == 0: # ベクトルが空なら
            new_vector = dataset_valid_D # ベクトルが空なら代入
          else: # 空じゃないなら
            # print("dataset_D: ", dataset_D)
            dataset_valid_D = np.delete(dataset_valid_D, 90, 1) # 日にちがダブるから削除
            dataset_valid_D = dataset_valid_D.reshape([1, 91]) # dataset_D を(0, 91)から(1, 91)にする
            # print("dataset_D.shape: ", dataset_D.shape)
            # print("new_vector.shape: ", new_vector.shape)
            new_vector = np.concatenate([new_vector, dataset_valid_D], axis = 1)
            # print("new_vector.shape: ", new_vector.shape)
            # new_vector = pd.concat([new_vector, dataset_D], axis = 1) # 空じゃないなら結合
            
          # print("\n")
      
      new_dataset_valid[n, :] = new_vector # 新しく作ったベクトルをデータセットに代入する
      n = n + 1 #代入する際のカウンター
      
      end_lonLat = time()
      cost_lonLat = np.round( (end_lonLat - start_lonLat)/60 , 2)
      # print(f"dataset_valid.date = {i} (Lon_Lat) Finished in : {cost_lonLat} minutes" ) # 各地点の処理時間の長さ
...
print("new_dataset_valid.shape: \n", new_dataset_valid.shape)

pickle.dump(new_dataset_valid, open('new_dataset_valid_Spatial_Correlation.pkl', 'wb')) 
dataset_valid = pickle.load(open('new_dataset_valid_Spatial_Correlation.pkl', 'rb')) # 新しい方のdataset_valid
